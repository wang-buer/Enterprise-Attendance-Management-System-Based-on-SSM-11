	...
	layui.use('table', function(){
		  ...
		  var dictable=table.render({
		    elem: '#test'
		   	,width: 1100
		    ,height: 350
		    ,url: '<%=request.getContextPath()%>/login/userList.do' //数据接口
		    ,limit: 10
	         ,id:'id'
		    ,cols: [[ //表头
		       {type:'checkbox'}
		      ,{field: 'id', title: 'ID', width:80,sort: true , align:'center'}
		      ,{field: 'job_number', title: '员工工号', width:160 , align:'center'}
		      ,{field: 'login_name', title: '用户名', width:160 , align:'center'}
		      ,{field: 'loginId', title: '登陆名称', width:160, sort: true , align:'center'}
		      ,{field: 'passWord', title: '登陆密码', width:160 , align:'center'} 
		      ,{field: 'role_name', title: '所属权限', width: 160 , align:'center'}
		  ...
		  var $ = layui.$, active={
					  reload:function () {
						 var demoReload = $('#demoReload').val();
			              ...
			              dictable.reload(
			             {
			            	  url: '<%=request.getContextPath()%>/login/userList.do'
			          ...
			          addUser:function(){
			        	  	parent.layer.open({
			    		        ...
			    		        area: ['500px', '400px'],
			    		        skin: 'demo-class',
			    				  content: '<%=request.getContextPath()%>/login/selectList.do?statusAdd=2',
		  ...
		  table.on('tool(test)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
		    ...
		    if(layEvent === 'detail'){
		      ...
		      layer.confirm('真的删除行么', function(index){
		        ...
		        //向服务端发送删除指令
		        $.ajax({
                    url:'<%=request.getContextPath()%>/login/delUser.do',
                    type:'get',
                    dataType:'json',
                    data:{
                    	userId:data.id
		      ...
		      var url="<%=request.getContextPath()%>/login/selectList.do?id="+data.id+"&job_number="+data.job_number+"&login_name="+data.login_name+"&loginId="+data.loginId+"&passWord="+data.passWord+"&role_name="+data.role_name ;
		  	parent.layer.open({
				type: 2,
...
<body>
  <div align="center" class="container">
		...
		<div class="layui-inline">   
				<input class="layui-input" maxlength="15" name="keyword" id="demoReload" autocomplete="off"> 			
		 </div>
   		</td>
   				<td>
   		...
   		<td>
		<div class="layui-inline">   
				<input class="layui-input" maxlength="10" name="keywordname"  id="demoReloadName" autocomplete="off"> 			
		 </div>
   		</td>
   		<td>
